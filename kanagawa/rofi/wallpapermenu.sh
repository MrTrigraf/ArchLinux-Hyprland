...
wallpaper_dir="/home/barmaley/.config/hypr/kanagawa/wallpaper"

find "$wallpaper_dir" -type f \( -iname "*.jpg" -o -iname "*.png" -o -iname "*.jpeg" -o -iname "*.gif" \) -print0 | \
while IFS= read -r -d $'\0' file; do
    printf "%s\0icon\x1f%s\n" "$(basename "$file")" "$file"
done | \
rofi -dmenu -i -p "wallpaper" -markup-rows -width 40 -lines 15 -theme ~/.config/hypr/kanagawa/rofi/wallpaper-switcher.rasi | \
